...
const SortableItem = ({ section, config }: { section: any, config: any }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-3 bg-white p-3 rounded-xl border-2 border-gray-50 shadow-sm mb-2 group transition-all hover:border-blue-100">
      <div {...attributes} {...listeners} className="cursor-grab text-gray-300 hover:text-gray-600 transition-colors">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Column {config.column || 1}</span>
      </div>
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button 
            ...
            className={cn(
                ...
                config.column === 2 ? "bg-blue-50 text-blue-600" : "text-gray-400 hover:bg-gray-50 hover:text-gray-600"
            )}
            title="Switch Column"
        >
            <Columns size={14} />
      ...
      >
        <div className="p-1">
          {sectionConfigs.map((config) => {
            ...
            return <SortableItem key={section.id} section={section} config={config} />;
          })}
        </div>
      </SortableContext>
    </DndContext>
