  ...
  return nodes.map(node => {
    ...
    if (node.marks) {
      ...
      const orderedMarks: Mark[] = [
        ...(node.marks.filter(m => typeof m === 'string') as string[]).sort(),
      ...
      orderedMarks.forEach(mark => {
        if (typeof mark === 'string') {
          switch (mark) {
            case 'bold': children = `<strong>${children}</strong>`; break;
            case 'italic': children = `<em>${children}</em>`; break;
            case 'underline': children = `<u>${children}</u>`; break;
            case 'strikethrough': children = `<s>${children}</s>`; break;
          }
...
export function htmlToTextNodes(html: string): TextNode[] {
  ...
  function traverse(node: Node, activeMarks: Mark[] = []) {
    ...
    switch (tagName) {
      case 'strong': case 'b': newMarks.push('bold'); break;
      case 'em': case 'i': newMarks.push('italic'); break;
      case 'u': newMarks.push('underline'); break;
      case 's': newMarks.push('strikethrough'); break;
      case 'a': newMarks.push({ type: 'link', attrs: { href: (element as HTMLAnchorElement).href } }); break;
    }
  ...
  if (nodes.length > 0) {
    ...
    for (let i = 1; i < nodes.length; i++) {
      ...
      const marksAreEqual = JSON.stringify(last.marks) === JSON.stringify(current.marks);
